...
type Link struct {
	...
	Ahref null.String `boil:"ahref" json:"ahref,omitempty" toml:"ahref" yaml:"ahref,omitempty"`
...
var LinkWhere = struct {
	...
	Ahref whereHelpernull_String
}{
	ID:    whereHelperint{field: "\"links\".\"id\""},
	Name:  whereHelpernull_String{field: "\"links\".\"name\""},
	Ahref: whereHelpernull_String{field: "\"links\".\"ahref\""},
}

// LinkRels is where relationship names are stored.
var LinkRels = struct {
	CidCategories string
}{
	CidCategories: "CidCategories",
}

// linkR is where relationships are stored.
type linkR struct {
	CidCategories CategorySlice `boil:"CidCategories" json:"CidCategories" toml:"CidCategories" yaml:"CidCategories"`
}
...
var (
	linkAllColumns            = []string{"id", "name", "ahref"}
	linkColumnsWithoutDefault = []string{"id", "name", "ahref"}
...
// CidCategories retrieves all the category's Categories with an executor via id column.
func (o *Link) CidCategories(mods ...qm.QueryMod) categoryQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"links_categories\" on \"categories\".\"id\" = \"links_categories\".\"cid\""),
		qm.Where("\"links_categories\".\"lid\"=?", o.ID),
	)
...
// LoadCidCategories allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (linkL) LoadCidCategories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLink interface{}, mods queries.Applicator) error {
	var slice []*Link
	var object *Link
	...
	query := NewQuery(
		qm.Select("\"categories\".id, \"categories\".name, \"a\".\"lid\""),
		qm.From("\"categories\""),
		qm.InnerJoin("\"links_categories\" as \"a\" on \"categories\".\"id\" = \"a\".\"cid\""),
		qm.WhereIn("\"a\".\"lid\" in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.CidCategories = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &categoryR{}
			}
			foreign.R.LidLinks = append(foreign.R.LidLinks, object)
		}
		return nil
	}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.CidCategories = append(local.R.CidCategories, foreign)
				if foreign.R == nil {
					foreign.R = &categoryR{}
				}
				foreign.R.LidLinks = append(foreign.R.LidLinks, local)
				break
			}
		}
	}
...
// AddCidCategories adds the given related objects to the existing relationships
// of the link, optionally inserting them as new records.
// Appends related to o.R.CidCategories.
// Sets related.R.LidLinks appropriately.
func (o *Link) AddCidCategories(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Category) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"links_categories\" (\"lid\", \"cid\") values ($1, $2)"
	...
	if o.R == nil {
		o.R = &linkR{
			CidCategories: related,
		}
	} else {
		o.R.CidCategories = append(o.R.CidCategories, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &categoryR{
				LidLinks: LinkSlice{o},
			}
		} else {
			rel.R.LidLinks = append(rel.R.LidLinks, o)
		}
	}
	return nil
}

// SetCidCategories removes all previously related items of the
// link replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.LidLinks's CidCategories accordingly.
// Replaces o.R.CidCategories with related.
// Sets related.R.LidLinks's CidCategories accordingly.
func (o *Link) SetCidCategories(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Category) error {
	query := "delete from \"links_categories\" where \"lid\" = $1"
	...
	removeCidCategoriesFromLidLinksSlice(o, related)
	if o.R != nil {
		o.R.CidCategories = nil
	}
	return o.AddCidCategories(ctx, exec, insert, related...)
}

// RemoveCidCategories relationships from objects passed in.
// Removes related items from R.CidCategories (uses pointer comparison, removal does not keep order)
// Sets related.R.LidLinks.
func (o *Link) RemoveCidCategories(ctx context.Context, exec boil.ContextExecutor, related ...*Category) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"links_categories\" where \"lid\" = $1 and \"cid\" in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.CidCategories {
			if rel != ri {
				continue
			}

			ln := len(o.R.CidCategories)
			if ln > 1 && i < ln-1 {
				o.R.CidCategories[i] = o.R.CidCategories[ln-1]
			}
			o.R.CidCategories = o.R.CidCategories[:ln-1]
			break
		}
	}
...
func removeCidCategoriesFromLidLinksSlice(o *Link, related []*Category) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.LidLinks {
			if o.ID != ri.ID {
				continue
			}

			ln := len(rel.R.LidLinks)
			if ln > 1 && i < ln-1 {
				rel.R.LidLinks[i] = rel.R.LidLinks[ln-1]
			}
			rel.R.LidLinks = rel.R.LidLinks[:ln-1]
			break
		}
	}
}
