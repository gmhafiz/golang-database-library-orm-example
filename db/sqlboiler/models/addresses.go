...
func (addressL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAddress interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"users\".id, \"users\".first_name, \"users\".middle_name, \"users\".last_name, \"users\".email, \"users\".password, \"users\".favourite_colour, \"users\".tags, \"users\".updated_at, \"a\".\"address_id\""),
		qm.From("\"users\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.FirstName, &one.MiddleName, &one.LastName, &one.Email, &one.Password, &one.FavouriteColour, &one.Tags, &one.UpdatedAt, &localJoinCol)
		if err != nil {
