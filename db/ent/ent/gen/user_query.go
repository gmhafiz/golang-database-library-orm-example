...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withAddresses; query != nil {
		...
		ids := make(map[uint]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Addresses = []*Address{}
		}
		var (
			edgeids []uint
			edges   = make(map[uint][]*User)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.AddressesPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "addresses": %w`, err)
		}
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Addresses = append(nodes[i].Edges.Addresses, n)
			}
		}
	}
